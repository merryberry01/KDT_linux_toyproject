...
int input()
{
   ...
   /* 여기서 스레드를 생성한다. */
    if(pthread_create(&command_thread_tid, NULL, command_thread, NULL) == -1)
        perror("pthread_create(command_thread) in input()");
    if(pthread_create(&sensor_thread_tid, NULL, sensor_thread, NULL) == -1)
        perror("pthread_create(sensor_thread) in input()");

    while (1) {
        sleep(1);
    }
    
    if(pthread_join(command_thread_tid, NULL) == -1)
        perror("pthread_join(cmomand_thread) in input()");
    if(pthread_join(sensor_thread_tid, NULL) == -1)
        perror("pthread_join(sensor_thread) in input()");
